...
filter ::
  ...
  -> List a
  -> List a
filter f = foldRight (\x y -> if f x == True then (x :. y) else y) Nil
...
seqOptional (x :. xs) = 
    case x of 
        (Full val) -> concatList (Full (val :. Nil)) (seqOptional xs)
        Empty -> Empty

concatList :: Optional (List a) -> Optional (List a) -> Optional (List a)
concatList Empty _ = Empty
concatList _ Empty = Empty
concatList (Full ls1) (Full ls2) = Full (ls1 ++ ls2)
