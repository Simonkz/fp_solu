...
instance Applicative ExactlyOne where
  ...
  (<*>) :: 
    ...
    -> ExactlyOne a
    -> ExactlyOne b
  (<*>) (ExactlyOne fab) (ExactlyOne v) = ExactlyOne (fab v)
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA k _ | k <= 0 = pure Nil
replicateA k fa = (:.) <$> fa <*> replicateA (k-1) fa
...
filtering f la = helper <$> pure la <*> lb 
                    where lb = (sequence . map f) la

helper :: List a -> List Bool -> List a
helper Nil _ = Nil
helper _ Nil = Nil
helper (x :. xs) (b :. bs) = 
    if b == True 
