...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f sa = State $ (\(va, vs) -> (f va, vs)) . runState sa
...
instance Applicative (State s) where
  ...
  (<*>) sab sa = 
    State $ \s -> 
      let (fab, stateAB) = runState sab s
          (varA, stateA) = runState sa stateAB
      in (fab varA, stateA)
...
instance Monad (State s) where
  ...
  (=<<) fAtoSb sa =
    State $ \s -> 
      let (varA, stateA) = runState sa s
      in runState (fAtoSb varA) stateA
...
-- S.insert 
--   :: Ord a => a -> Set a -> Set a
-- lift2 (lift2 (,)) S.member S.insert
--   :: Ord a => a -> Set a -> (Bool, Set a)
-- State . lift2 (lift2 (,)) m S.insert
--   :: Ord a => a -> State (Set a) Bool
-- instance Applicative ( (->) (a -> Set a) ) 
-- It is function applicative, haha. 
...
-- | where the sum of the square of its digits 
-- | eventually reaches 1 after repetition.
-- | In contrast, a sad number (not a happy number) is 
-- | where the sum of the square of its digits 
-- | never reaches 1
