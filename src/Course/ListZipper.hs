...
findLeft f (ListZipper ls x rs) =
  let (lss, rss) = break f ls
  in case rss of 
    Nil -> IsNotZ
    (xr :. xrs) -> IsZ $ ListZipper xrs xr (reverse lss ++ x :. Nil ++ rs)
...
findRight f (ListZipper ls x rs) =
  let (lss, rss) = break f rs
  in case rss of 
    Nil -> IsNotZ
    (xr :. xrs) -> IsZ $ ListZipper (reverse lss ++ x :. Nil ++ ls) xr xrs
...
moveLeftLoop (ListZipper Nil x rs) = 
  case reverse rs ++ x :. Nil of
    Nil -> ListZipper Nil x rs
    (p :. ps) -> ListZipper ps p Nil
moveLeftLoop (ListZipper (xl :. xls) x rs) =
  ListZipper xls xl (x :. Nil ++ rs)
...
moveRightLoop (ListZipper ls x Nil) =
    case reverse ls ++ x :. Nil of
      Nil -> ListZipper ls x Nil
      (p :. ps) -> ListZipper Nil p ps
moveRightLoop (ListZipper ls x (xr :. xrs)) =
  ListZipper (x :. Nil ++ ls) xr xrs
...
moveLeft (ListZipper ls x rs) =
  case ls of 
    Nil -> IsNotZ
    (xl :. xls) -> IsZ $ ListZipper xls xl (x :. Nil ++ rs) 
...
moveRight (ListZipper ls x rs) =
  case rs of
    Nil -> IsNotZ
    (xr :. xrs) -> IsZ $ ListZipper (x :. Nil ++ ls) xr xrs
...
swapLeft (ListZipper ls x rs) =
  case ls of
    (xl :. xls) -> IsZ $ ListZipper (pure x ++ xls) xl rs
    Nil -> IsNotZ
...
swapRight (ListZipper ls x rs) =
  case rs of 
    (xr :. xrs) -> IsZ $ ListZipper ls xr (pure x ++ xrs)
    Nil -> IsNotZ
...
moveLeftN' n lz
  | n == 0 = Right lz
  | n < 0 = moveRightN' (-n) lz
  | n > (length $ lefts lz) = Left $ length $ lefts lz
  | otherwise = 
...
moveRightN' n lz 
  | n == 0 = Right lz
  | n < 0 = moveLeftN' (-n) lz
  | n > (length $ rights lz) = Left $ length $ rights lz
  | otherwise = 
...
deletePullLeft (ListZipper ls _ rs) =
  case ls of 
    Nil -> IsNotZ
    (xl :. xls) -> IsZ $ ListZipper xls xl rs 
...
deletePullRight (ListZipper ls _ rs) =
  case rs of 
    Nil -> IsNotZ
    (xr :. xrs) -> IsZ $ ListZipper ls xr xrs 
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft x (ListZipper ls p rs) =
  ListZipper (pure p ++ ls) x rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight x (ListZipper ls p rs) =
  ListZipper ls x (pure p ++ rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper fls f frs) (ListZipper ls x rs) =
    ListZipper (zipWith ($) fls ls) (f x) (zipWith ($) frs rs)
...
-- This implementation "visits" every possible zipper value 
-- derivable from a given zipper (i.e. all zippers to the left and right).
--
...
-- (<<=) :: (ListZipper a -> b) -> ListZipper a -> ListZipper b
  (<<=) f lz =
    ListZipper (unfoldr (optF moveLeft f) lz) (f lz) (unfoldr (optF moveRight f) lz)
    where optF mv f' x = 
            case mv x of 
              (IsZ newX) -> Full $ (,) (f' newX) newX
              IsNotZ -> Empty
